class TombstoneGame {
    ...
    startGame() {
        ...
        setTimeout(() => {
            ...
            if (window.PixelTombstoneGame) {
                ...
                this.pixelGame.selectedCharacter = this.selectedCharacter;
    ...
    initializeMultiplayerGame(gameData) {
        ...
        // Initialize the pixel game
        if (window.PixelTombstoneGame) {
            this.pixelGame = new PixelTombstoneGame();
            this.pixelGame.selectedCharacter = this.selectedCharacter;
            this.pixelGame.isMultiplayer = true;
            this.pixelGame.multiplayerManager = this.multiplayerManager;
            ...
            // Set opponent character and positioning
            const opponent = players.find(p => p.playerId !== this.multiplayerManager.playerId);
            console.log('🎭 MULTIPLAYER: Setting opponent character...', { opponent, hasCharacter: !!opponent?.character });
            if (opponent && opponent.character) {
                this.pixelGame.opponentCharacter = opponent.character;
                console.log('✅ MULTIPLAYER: Opponent character set to:', opponent.character);
            } else {
                console.warn('⚠️ MULTIPLAYER: No opponent character found!');
            }

            // Configure multiplayer positioning
            this.pixelGame.isPlayerOnLeft = isPlayerOnLeft;
