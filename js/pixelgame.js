class PixelTombstoneGame {
    constructor() {
        ...
        this.isPlayerOnLeft = true; // Default to left side
    ...
    animateCharacterShoot(characterElement, character) {
        ...
        setTimeout(() => {
            ...
            }, 300); // Hold shooting pose for 300ms
        }, 150); // Draw for 150ms before shooting
    }
    ...
    createBullet(fromPlayer) {
        ...
        // Position bullet at character height (middle of screen)
        ...
        if (fromPlayer) {
            ...
            bullet.classList.add('fly-left');
            trail.classList.add('trail-left');
        } else {
            // Opponent shoots from right to left
            bullet.classList.add('fly-right');
            trail.classList.add('trail-right');
        }
    ...
    triggerBell() {
        ...
        setTimeout(() => {
            ...
            if (!this.isMultiplayer) {
                ...
                console.log('Multiplayer mode: AI disabled, waiting for real player actions');
            }
        }, 1000);
    }
    ...
    opponentShoot() {
        ...
        if (hit) {
            ...
            // Flash player character red when hit
    ...
    playerShoot(x, y) {
        ...
        if (hit) {
            ...
            if (this.isMultiplayer) {
                ...
                // Flash opponent character red when hit
        ...
        if (this.opponentHealth > 0) {
            setTimeout(() => {
                if (this.gameState === 'shooting' && this.opponentHealth > 0 && this.playerHealth > 0) {
                    ...
                    // Restart opponent AI for next round
                    this.startOpponentAI();
                }
            }, 1000);
        }
    ...
    endGame(winner) {
        ...
        if (winner === 'player') {
            ...
            if (playerSprite) {
                ...
                playerSprite.style.filter = 'grayscale(100%)';
            }
        }
    ...
    playerDraw() {
        ...
        if (playerElement) {
            ...
            // Return to idle after drawing animation
            setTimeout(() => {
                this.setCharacterImage(playerElement, this.selectedCharacter, 'idle');
            }, 500);
        }
